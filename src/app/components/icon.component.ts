...
export class NGXSeasonIconComponent implements OnChanges, AfterViewInit {
    ...
    protected setupIconContent(shape: string, solid: boolean): void {
        this.svgContent$ = this._register.findIcon(shape).pipe(map(pair => 
            this._sanitizer.bypassSecurityTrustHtml(solid ? pair.solid : pair.outline)));
    }
