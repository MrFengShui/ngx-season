...
@Component({
    ...
    template: `
        <a class="node-control" (click)="handleNodeControlEvent($event, model?.data)">
            <button ngx-sui-FlatIconButton [btnColor]="_tree.color" [btnIcon]="toggled ? (collapseIcon || _tree.collapseIcon) : (expandIcon || _tree.expandIcon)" btnCircled="true" btnSize="md" [style.visibility]="hasChildren(model?.children) ? 'visible' : 'hidden'" (click)="handleNodeToggleEvent($event)" *ngIf="_tree.ctrlStyle === 'flat'"></button>
            <button ngx-sui-OutlineIconButton [btnColor]="_tree.color" [btnIcon]="toggled ? (collapseIcon || _tree.collapseIcon) : (expandIcon || _tree.expandIcon)" btnCircled="true" btnSize="sm" [style.visibility]="hasChildren(model?.children) ? 'visible' : 'hidden'" (click)="handleNodeToggleEvent($event)" *ngIf="_tree.ctrlStyle === 'outline'"></button>
            <button ngx-sui-SolidIconButton [btnColor]="_tree.color" [btnIcon]="toggled ? (collapseIcon || _tree.collapseIcon) : (expandIcon || _tree.expandIcon)" btnCircled="true" btnSize="sm" [style.visibility]="hasChildren(model?.children) ? 'visible' : 'hidden'" (click)="handleNodeToggleEvent($event)" *ngIf="_tree.ctrlStyle === 'solid'"></button>
